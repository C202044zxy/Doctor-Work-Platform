...
async def handle(app, room, user, connection, kind, payload):
    from app.chat import room_for

    body = Signal.model_validate(payload)
    hub = app.state.chat
    with app.state.sessions() as db:
        db.info["user"] = user
        session = room_for(db, room, participant=True)
        if session.status != "active":
            raise HTTPException(409, "Video calls require an active consultation")
