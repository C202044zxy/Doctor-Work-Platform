...
### 4.1 M7 范围调整（2026-09-23）

| # | 需求 | 结论 | 原因 | 答辩口径 |
|---|---|---|---|---|
| 1 | **医生社区（M7）** | 2026-09-23 用户要求纳入基础论坛 | 新增范围见 docs/01 §3.8 | 独立表、同一 SQLite 文件，无患者关联，示例内容虚构。 |

> M7 原为二期，现按用户要求新增基础论坛。下面 4.2 的六条以"模拟实现"纳入本期范围。
...
| 方法 | 路径 | 用途 |
|---|---|---|
| GET / POST | `/api/forum/posts` | 按 q/category 搜索分页 / 新建讨论 |
| GET | `/api/forum/posts/{post_id}` | 讨论详情与回复数 |
| GET / POST | `/api/forum/posts/{post_id}/replies` | 回复分页（最早优先）/ 发布回复 |

列表均含 items/total/page/size；创建成功 200；匿名 401、缺失或模块关闭 404、校验失败 422。Sample 由后端 seed_key 判定，客户端不可冒充。写入审计不含正文。前端 `/forum` 从侧栏 Medical Forum 进入。
